...
**16** Chapter 2 ■ Understanding Your AWS Account
...
T Able 3.1 Monthly Pricing for Amazon Paid Support Plans (All prices in U.S. Dollars)
...
f nd i out how to Copy files from one S3 bucket to Another
...
**58** Chapter 4 ■ Understanding the AWS Environment
...
**1.** Choose the CloudFront link from the AWS services menu, choose Create Distribution,  
...
**2.** Once you’re on the Create Distribution page, choose inside the box next to the Origin  
...
**3.** Scroll down the page and note, for instance, the Distribution Settings section. Choose  
...
functionality by adding what it calls a _regional edge cache_. The idea is that CloudFront-  
...
Amazon distinguishes between the security and reliability _of_ the cloud, which is its  
responsibility, and the security and reliability of what’s _in_ the cloud, which is up to you,  
the customer.  
...
**60** Chapter 4 ■ Understanding the AWS Environment

```
f gi UrE 4.3 A general comparison between local and managed deployments
```
...
```
f gi UrE 4.4 A representation of the AWS Shared Responsibility Model
```
...
demand less. Use this simple rule of thumb: _if you can edit it, you own it_. The key—espe-  
...
**62** Chapter 4 ■ Understanding the AWS Environment
...
**C.** Availability Zone  
**D.** Region  
**4.** Which of the following are _not_ globally based AWS services? (Select TWO.)
...
**13.** Which of the following design strategies is _most_ effective for maintaining the reliability of a  
cloud application?  
**A.** Resource isolation  
**B.** Resource automation  
**C.** Resource redundancy  
**D.** Resource geolocation  
**14.** Which of the following AWS services are _not likely_ to benefit from Amazon edge locations?  
(Select T WO.)  
**A.** RDS  
...
about catastrophic data breaches suggests that some of us may not yet realize just _how_  
important.  
...
You’ll learn how to use the _Identity and Access Management_ (IAM) service to control  
...
We’ll discuss applying _encryption_ to your cloud data—both while it’s on its way from  
...
the _effective_ functionality of root by setting up IAM users and groups for your day-to-day  
...
f gui Re 5.1 The Security Status section of the IAM Dashboard includes important  
...
**70** Chapter 5 ■ Securing Your AWS Resources
...
```
figuRe  5.2 The IAM Account Settings page where you can set an account-wide
password policy
```
...
**1.** From the IAM Dashboard, choose Account Settings, which will take you to a page  
that includes the Password Policy tool.  
**2.** If appropriate for your account, change the value of Minimum Password Length from  
6 to 8. This will require all new passwords to contain at least eight characters.  
**3.** Again, if appropriate, check these boxes:
...
of keys while logged into the AWS Management Console from the _Security Credentials_  
...
**72** Chapter 5 ■ Securing Your AWS Resources
...
**74** Chapter 5 ■ Securing Your AWS Resources
...
already seen.  
You can integrate third-party standards like the _Security Assertion Markup Language 2.0_  
(SAML) or Microsoft’s Active Directory into your infrastructure. This lets you use users’  
existing login sessions to add _single sign-on_ (SSO) across your AWS infrastructure and  
...
transformation—are mostly managed on AWS by the AWS _Key Management Service_  
...
**76** Chapter 5 ■ Securing Your AWS Resources
...
**D.** SSE-KMS  
**16.** Which of the following AWS resources _cannot_ be encrypted using KMS?  
...
The _AWS Management Console_ , also known as the AWS Console or the AWS Web  
...
**86** Chapter 6 ■ Working with Your AWS Resources

```
f gui Re 6.1 Logging in as a root user
```
...
```
f gui Re 6.2 Entering the account alias to log in as an IAM User
```
...
```
The AWS Management Console 87
```

f gui Re 6.3 Logging in as an IAM user
...
f gui Re 6.4 Browsing available service consoles

**88** Chapter 6 ■ Working with Your AWS Resources
...
**90** Chapter 6 ■ Working with Your AWS Resources
...
```
f gui Re 6.8 The account name menu when you’re logged in as an IAM user
```
...
```
The AWS Management Console 91
```

f gui Re 6.9 The Resource Groups menu
...
**Resource tags** _Resource tags_ are optional metadata that you can assign to AWS resources.  
A tag must contain a label called a _key_ and may optionally contain a value. For example,  
...
-   -   =. _ : / @. You can assign multiple tags to a resource, up to 50. Tag keys and values  
...
AWS resources as a single unit called a _stack_. You can create resource groups that contain  
...
**92** Chapter 6 ■ Working with Your AWS Resources
...
**94** Chapter 6 ■ Working with Your AWS Resources
...
```
f gui Re 6.11 The AWS Console Mobile Application dashboard
```

```
The AWS Management Console 95
```
...
f gui Re 6.12 Viewing a CloudWatch alarm from the AWS Console Mobile Application

```
Or you can stop or reboot an EC2 instance, as shown in Figure 6.13.
```

**96** Chapter 6 ■ Working with Your AWS Resources

```
f gui Re 6.13 Viewing an EC2 Instance from the AWS Console Mobile Application
```
...
f gui Re 6.14 Adding an identity to the AWS Console Mobile Application
...
**98** Chapter 6 ■ Working with Your AWS Resources

```
f gui Re 6.15 S3 bucket policy configuration, as shown in the AWS Console Mobile
Application
```
...
```
The AWS Command Line Interface 99
```
...
**100** Chapter 6 ■ Working with Your AWS Resources
...
```
i stall the AWS Command n line interface
```
...
**2.** In the AWS Management Console, generate an access key ID and secret key for an  
IAM user.  
**3.** In your terminal, run the command **aws configure** and enter the access key ID and  
...
**102** Chapter 6 ■ Working with Your AWS Resources
...
buttons.
AW S IoT is a collection of services that allow IoT devices to interact with AWS ser-
...
_Amazon CloudWatch_ is a key service that helps you plan, monitor, and fine-tune your  
...
and non-AWS resources such as on-premises servers. A _metric_ is a variable that contains a time-  
...
f gui Re 6.16 Using CloudWatch to graph the CPU Utilization metric for an EC2 Instance

**104** Chapter 6 ■ Working with Your AWS Resources
...
f gui Re 6.17 A CloudWatch dashboard
...
event message. Many AWS services produce their own logs called _vended logs_ that you can  
...
**106** Chapter 6 ■ Working with Your AWS Resources
...
IAM user, or taking an EBS snapshot. Note that the term _API action_ has nothing to do  
...
events.  
Management events—also known as _control plane operations_ —are operations that  
...
**108** Chapter 6 ■ Working with Your AWS Resources
...
f gui Re 6.18 Cost and usage report showing monthly costs

**110** Chapter 6 ■ Working with Your AWS Resources
...
```
f gui Re 6.19 Cost and usage report showing monthly costs grouped by service
```
...
f gui Re 6.20 RI Utilization report

**112** Chapter 6 ■ Working with Your AWS Resources
...
```
f gui Re 6.21 RI Coverage report
```
...
**114** Chapter 6 ■ Working with Your AWS Resources
...
centers: you provision and launch virtual servers (known as _instances_ ) to run the same  
...
An _image_ is a software bundle that was built from a template definition and made available  
within a single AWS Region. The bundle can be copied to a freshly created storage volume

```
Deploying Amazon Elastic Compute Cloud Servers 121
```
...
incur no costs.  
The available Linux distributions you’ll find here are mostly categorized as _long-term  
support_ (LTS) releases, meaning that they’ll be eligible for security and functional updates  
...
carry the normal _licensing charges_. Once you launch an instance running one of those  
...
f gure 7.1i A couple of EC2 AMI listings displaying features and options

**122** Chapter 7 ■ The Core Compute Services
...
```
Deploying Amazon Elastic Compute Cloud Servers 123
```
...
moderate data transfer rates to network connections, and one _virtual CPU_ (vCPU) running  
on a 2.5 GHz Intel Xeon processor. A c5d.18xlarge instance type, on the other hand, gives  
you 72 vCPUs on a 3 GHz Intel Xeon Platinum 8124M processor, and 144 GB of memory.  
...
uses. Besides _general-purpose_ , EC2 also offers _compute-optimized_ , _memory-optimized_ ,  
_accelerated-computing_ , and _storage-optimized_ families of instance types. Table 7.1 lists the  
...
T Able 7.1 EC2 Instance Type Families (At the Time of This Writing)
...
**124** Chapter 7 ■ The Core Compute Services
...
```
Deploying Amazon Elastic Compute Cloud Servers 125
```
...
**126** Chapter 7 ■ The Core Compute Services
...
```
l unch an Apache Web Server on an a eC2 instance
```
...
One step beyond a managed service—which handles only _one part_ of your deployment  
stack for you—is a managed _deployment_ , where the _whole stack_ is taken care of behind the  
...
**128** Chapter 7 ■ The Core Compute Services
...
with specially formatted Dockerrun.aws.json fi l e s.
...
_Container technologies_ such as Docker avoid a lot of that overhead by allowing individual  
containers to share the Linux kernel with the physical host. They’re also able to share com-  
mon elements (called _layers_ ) with other containers running on a single host. This makes  
...
other can get complicated. Instead, you can use either _Amazon Elastic Container Service_  
(ECS) or _Amazon Elastic Container Service for Kubernetes_ (EKS) to orchestrate swarms of  
...
left by containers. Not only do _serverless functions_ not require their own OS kernel, but  
...
**130** Chapter 7 ■ The Core Compute Services
...
types are divided into type _families_ , each of which focuses on a functional niche (general  
...
them, _objects_ on disks in AWS data centers. You can store any kind of fi le, including text,  
images, videos, database fi les, and so on. Each object can be up to 5 TB in size.  
The fi lename of an object is called its _key_ and can be up to 1,024 bytes long. An object  
key can consist of alphanumeric characters and some special characters, including the  
following:

! - _. * ( )

S3 stores objects in a container called a _bucket_ that’s essentially a fl at fi le system. A  
...
**140** Chapter 8 ■ The Core Storage Services
...
```
STANDARD_ IA 99.999999999% 99.9% >2 $0.0125
```

```
INTELLIGENT_TIERING 99.999999999% 99.9% >2 Frequent access
tier: $0.023
Infrequent
access tier:
$0.0125
...
**142** Chapter 8 ■ The Core Storage Services
...
ing them in S3. This is called _encryption at rest_. S3 gives you the following two options for  
...
**144** Chapter 8 ■ The Core Storage Services
...
**1.** In the S3 service console, choose the Create Bucket button.  
**2.** Enter a bucket name. Remember that this must be globally unique, so try to pick  
something that nobody else is likely to have used.  
**3.** Select a region of your choice, and then choose the Next button.  
**4.** Enable versioning by checking the Versioning check box, and then choose the Next button.  
**5.** Choose the Next button again to stick with the default settings. By default, objects in  
...
**146** Chapter 8 ■ The Core Storage Services
...
**148** Chapter 8 ■ The Core Storage Services
...
**150** Chapter 8 ■ The Core Storage Services

```
T Able 8.2 Comparison of Snowball and Snowball Edge
```
...
columns are called _attributes_ , and rows are called _records_. Both are stored in a _table_ , and a  
...
**160** Chapter 9 ■ The Core Database Services
...
**162** Chapter 9 ■ The Core Database Services
...
```
Amazon Relational Database Service 163
```
...
the number of _input/output operations per second_ (IOPS) the EBS volume supports. IOPS  
...
_bursting_. During spikes of heavy read or write activity, bursting will kick in automatically

**164** Chapter 9 ■ The Core Database Services
...
```
Amazon Relational Database Service 165
```

With multi-AZ enabled, RDS creates an additional instance called a _standby database  
instance_ that runs in a different Availability Zone than your primary database instance.  
...
Do you need snapshots _and_ multi-AZ? It’s important to understand that although both  
...
**166** Chapter 9 ■ The Core Database Services
...
```
T Able 9.2 A Sample DynamoDB Table
```
...
**Scalar** A _scalar data type_ has only one value and can be a string, a number, binary data,  
or a Boolean value.

**Set** A _set data type_ can have multiple scalar values, but each value must be unique within  
the set.

**Document** _The document data type_ is subdivided into two subtypes: list and map.  
...
To locate all items with a Username that starts with the letter _p_ , you’d have to perform a  
...
**168** Chapter 9 ■ The Core Database Services
...
**1.** In the AWS Management Console, browse to the DynamoDB Service Console.  
**2.** Choose the Create Table button.  
**3.** In the Table Name field, give your table the name **Foods**.  
**4.** In the Primary Key field, enter the word **FoodName**.  
...
**7.** In the Read/write Capacity Mode section, select the radio button next to On-Demand.  
...
**11.** n the field next to FoodName String, enter I **Asparagus**.  
...
T Able 9.3 Comparison of Relational and Nonrelational Databases
...
with any letter between _H_ and _N_. Nonrelational databases don’t offer this kind of query  
flexibility.  
...
**170** Chapter 9 ■ The Core Database Services
...
Each VPC requires a Classless Inter-Domain Routing ( _CIDR_ ) block to define the range of  
...
**178** Chapter 10 ■ The Core Networking Services
...
```
Virtual Private Cloud 179
```

## VPC Peering

A _VPC peering connection_ is a private, point-to-point connection between only two VPCs.  
...
**180** Chapter 10 ■ The Core Networking Services
...
```
T Able 10.1 Resource Records for the benpiper.com Domain
```
...
```
http://www.benpiper.com A - IPv4 address 93.184.216.34
```
...
**182** Chapter 10 ■ The Core Networking Services
...
this problem by caching your content in a number of data centers called _edge locations_. There  
...
**184** Chapter 10 ■ The Core Networking Services
...
Using code to define your infrastructure and configurations is commonly called the _infra-  
structure as code_ (IaC) approach. Defining IaC is a fundamental element of automation in  
...
**192** Chapter 11 ■ Automating Your AWS Workloads
...
e plore the Cloudx formation designer
...
**194** Chapter 11 ■ Automating Your AWS Workloads

```
e xe rCiSe 11.1 ( continued)
```

**3.** In the Choose A Template section, choose the Select A Sample Template drop-down,  
...
CodeBuild is a fully managed build service. A _build_ is a set of actions performed on source  
...
Automated testing is a key part of a software development practice called _continuous  
integration_ (CI). In CI, developers check their new or modifi ed code into a shared reposi-  
...
**196** Chapter 11 ■ Automating Your AWS Workloads
...
**198** Chapter 11 ■ Automating Your AWS Workloads
...
**200** Chapter 11 ■ Automating Your AWS Workloads
...
**202** Chapter 11 ■ Automating Your AWS Workloads
...
**204** Chapter 11 ■ Automating Your AWS Workloads
...
The _Well-Architected Framework_ is a set of principles that AWS recommends as a way of  
...
```
The Well-Architected Framework 211
```
...
**212** Chapter 12 ■ Common Use-Case Scenarios
...
**214** Chapter 12 ■ Common Use-Case Scenarios

```
f g i Ure 12.1 A highly available web application using Auto Scaling and elastic load
balancing
```
...
```
Availability Zone 1Availability Zone 2
```
...
A Highly Available Web Application Using Auto Scaling and Elastic Load Balancing **215**
...
198.51.100.100, you’d enter 19 8. 51.10 0.10 0/3 2. Refer to Figure 12.2 to see what your
...
**216** Chapter 12 ■ Common Use-Case Scenarios
...
A Highly Available Web Application Using Auto Scaling and Elastic Load Balancing **217**

f g i Ure 12.3 Application load balancer basic configuration

f g i Ure 12.4 Application load balancer Availability Zones configuration
...
**15.** n the Name field, type I **sample-web-app-target-group**.  
**16.** Next to Target Type, select the Instance radio button.

**218** Chapter 12 ■ Common Use-Case Scenarios
...
A Highly Available Web Application Using Auto Scaling and Elastic Load Balancing **219**
...
**1.** Browse to the CloudFormation service console. Make sure you’re in the AWS Region
...
**3.** Under Choose A Template, select the radio button titled Specify An Amazon S3

```
Template URL.
```

**4.** In the text fi eld, enter **https://s3.amazonaws.com/aws-ccp/launch-template.yaml**.  
**5.** Select the Next button.  
**6.** In the Stack Name fi eld, enter **sample-app-launch-template**.  
**7.** From the drop-down list, select the instance type you want to use, or stick with the
...
Auto Scaling will scale in. This is called a _target tracking policy_.
...
**220** Chapter 12 ■ Common Use-Case Scenarios
...
**8.** In the Subnet drop-down list, select at least two subnets. Refer to Figure 12.6 to get  
...
A Highly Available Web Application Using Auto Scaling and Elastic Load Balancing **221**

**11.** n the Target Groups field, select sample-web-app-target-group.I  
...
**222** Chapter 12 ■ Common Use-Case Scenarios
...
**3.** Give the bucket a name of your choice. Try for something that’s easy to type but  
unique (for this example, we used **benpiper2020** ). Then choose the Next button.  
**4.** Leave object versioning and encryption disabled by default. Enabling either won’t impact  
your ability to use the bucket for hosting a static website. Choose the Next button.

```
Static Website Hosting Using S3 223
```
...
**14.** n your favorite text editor, create a new file and populate it with any content you like. I  
For example, you can put “ **Hello, world!** ”. Save the file as index.html.  
**15.** n the S3 service console, choose the Overview tab.I  
...
**224** Chapter 12 ■ Common Use-Case Scenarios
...
**1.** Which of the following is _not_ one of the pillars of the Well-Architected Framework?  
...
**A.** [http://mygreatwebsite.s3-website-us-east-1.amazonaws.com](http://mygreatwebsite.s3-website-us-east-1.amazonaws.com/)  
**B.** [http://mygreatwebsite.s3.amazonaws.com](http://mygreatwebsite.s3.amazonaws.com/)  
**C.** [http://mygreatwebsite.s3-website-us-east.amazonaws.com](http://mygreatwebsite.s3-website-us-east.amazonaws.com/)  
**D.** [http://mygreatwebsite.s3-us-east-1.amazonaws.com](http://mygreatwebsite.s3-us-east-1.amazonaws.com/)
...
**15.** A. Version numbers are not publicly available, and the word _Current_ isn’t used in this context.  
...
interested only in the appropriate availability and quality of the overall _service_. The job of  
...
**17.** A, B. What’s _in_ the cloud is your responsibility—it includes the administration of  
...
**3.** C, D. The root user should _not_ be used for day-to-day admin tasks—even as part of an  
...
guarantee of the state of your own deployments. SOC reports are reports of audits _on_ AW S  
...
standards. They tangentially can also provide insight into best practices. They do _not_  
...
**254** Appendix B ■ Additional Services
