...
var ChatCbt = class {
  ...
  async _openai_chat(messages, apiKey, model) {
  ...
  async _ollama_chat(messages, ollamaUrl, model) {
...
var ChatCbtPlugin = class extends import_obsidian2.Plugin {
  async onload() {
    ...
    this.setStatusBarMode(this.settings.mode);
    this.addCommand({
      id: "chat",
...
var MySettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.createEl("br");
    new import_obsidian2.Setting(containerEl).setName("OpenAI API Key").setDesc("Create an OpenAI API Key from their website and paste here").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Ollama mode (local)").setDesc("Toggle on for a local experience if you are running Ollama").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.mode === "openai" ? false : true).onChange(async (value) => {
        ...
        await this.plugin.saveSettings();
        this.plugin.setStatusBarMode(this.plugin.settings.mode);
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Ollama server URL").setDesc("Edit this if you changed the default port for using Ollama").addText(
